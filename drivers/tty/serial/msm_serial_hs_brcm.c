...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	if (msm_uport->host_wakeup_level == 0) { /* host_wake is high */
		...
		/*handle close() but still have host wake case */
		if (msm_uport->clk_state == MSM_HS_CLK_PORT_OFF )  {
			spin_unlock_irqrestore(&uport->lock, flags);
			return IRQ_HANDLED;
			/*Do nothing when have call msm_hs_shutdown */
		}
...
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	unsigned long flags; // HTC_BT

	printk(KERN_INFO "[BT]=+ S DN +=\n");

	/* HTC_BT: move here to make disable host wake irq earlier */
	spin_lock_irqsave(&uport->lock, flags);
	if (use_low_power_wakeup(msm_uport))
		irq_set_irq_wake(msm_uport->wakeup.irq, 0);
	spin_unlock_irqrestore(&uport->lock, flags);
	...
	tasklet_kill(&msm_uport->tx.tlet);
#if 1 /* HTC_BT: fix Klocwork error */
	...
	/* HTC_BT: move unlock mutex here to avoid deadlock if msm_hs_wakeup_isr also occurs */
	mutex_unlock(&msm_uport->clk_mutex);
