...
/* +SSD_RIL: enable mux or not by product id */
...
#endif //defined(CONFIG_MONITOR_STREAMING_PORT_SOCKET) && defined(CONFIG_MSM_NONSMD_PACKET_FILTER)
...
#endif //HTC_ADD_RMNET_USB_RESET_RESUME
...
//--SSD_RIL
//++SSD_RIL:20140320: Fix null pointer issue, klockwork id: 1809, 1811, 5640
	if ( !skb ) {
...
static void rmnet_usb_rx_complete(struct urb *rx_urb)
{
	...
	if (!rx_urb->status && dev->data[4]) {
		...
		//	pr_err("%s: skb->dev->name: %s, entry->dev->name: %s, unet_id: %d, mux_id: %d",__func__,(char *)skb->dev->name,(char *)entry->dev->net->name, unet_id, mux_id);
