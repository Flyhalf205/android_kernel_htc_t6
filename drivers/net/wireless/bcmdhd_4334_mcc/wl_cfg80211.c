		...
		if (timeout > 0 && (!wl_get_p2p_status(wl, IF_ADD))) {
			...
			if (net_attach && !net_attach(wl->pub, _ndev->ifindex)) {
				...
				if (type == NL80211_IFTYPE_P2P_CLIENT){
                    ...
                    /*HTC Start - add enable otg lock for p2p*/
                    set_otg_clk(1);
                    /*HTC End*/
                }
...
#endif /* !WL_CFG80211_P2P_DEV_IF && (LINUX_VERSION < VERSION(3, 6, 0)) */
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 4, 0) || defined(OLD_CFG_80211) /* BRCM TBD */
	...
	bool aborted, bool fw_abort)
{
	...
	if (fw_abort && !in_atomic() && !check_hang_already(dev)) { /* BRCM TBD */
		wl_cfg80211_scan_abort(wl);
	}
...
static s32 wl_init_priv(struct wl_priv *wl)
{
	...
	if (err)
		goto wl_init_priv_fail; /* BRCM TBD */
