...
static void batt_level_adjust(unsigned long time_since_last_update_ms)
{
	...
	if (!prev_batt_info_rep->charging_enabled &&
		...
		if (is_voltage_critical_low(htc_batt_info.rep.batt_vol)) {
			...
			if ((allow_drop_one_percent_flag == false)
				...
				if (htc_batt_info.rep.pj_src > 0
						...
						/* allow level increase when PJ exist. */
			...
			/* To reduce the difference (2%/1min) between UI & SOC
		...
		if (htc_batt_info.rep.level == 0) {
			...
			/*
			* To avoid shutdown in high voltage. T6 issues due to battery characters
			* HW/Dean Yang, Shining request. If voltage higher than expected
			* voltage (ex.3.4V) & 0%, keep 1%
			*/
	...
	/* To store UI SOC into memory, but the moment of writing UI SOC
	 * into emmc is while executing shutdown.
	 */
...
static void batt_update_limited_charge(void)
{
	if ((htc_batt_info.state & STATE_EARLY_SUSPEND)
		|| (context_state & CONTEXT_STATE_BIT_DAYDREAM)) {
		/* screen OFF or in the Daydream state*/
...
static void sw_safety_timer_check(unsigned long time_since_last_update_ms)
{
	...
	/* check if battery charging is enabled in external charger */
...
static void power_jacket_level_update(int first)
{
	...
	/* Use discharge table to calculate level when FET open. */
	...
	if (htc_batt_info.rep.pj_full) {
		/* PMA: set level 100 when charging full */
		htc_batt_info.rep.pj_level = 100;
	} else if (htc_batt_info.rep.pj_vol < 2400) {
		/* PMA: After initial percentage of PJ comes out, Vjk keep previous the value of Vjk if Vjk < 2.4V */
		...
		if (htc_batt_info.rep.charging_enabled == 0) {
			...
			/* PMA: During discharging, CBI update every 10 minute and % drop allowed is 19% */
		...
		/* Charging algorithm */
			/* PMA: During charging, CBI update every 10 minute and % adding allowed is 19% */
			if (htc_batt_info.rep.pj_level - htc_batt_info.rep.pj_level_pre > 19)
				htc_batt_info.rep.pj_level = htc_batt_info.rep.pj_level_pre + 19;
		}
	}

	/* Limit level range in 0~100 */
...
void power_jacket_info_update(void)
{
	...
	if (htc_batt_info.rep.pj_src) {
		...
		if (!prev_pj_src || first) {
			/* Update level directly when PJ insert */
		...
		if (MSPERIOD(start_ktime, end_ktime) > 0 ||
			...
			queue_delayed_work(htc_batt_timer.batt_wq,
					...
					msecs_to_jiffies(PJ_LEVEL_DETECT_PERIOD)); // 30s
		} else if (htc_batt_info.rep.charging_source > 0) {
			if ((chg_dis_reason & chg_dis_pj_mask) || charger_dis_temp_fault) {
				pj_chg_status = PJ_CHG_STATUS_OFF;
			} else {
				/*	pj_full_status: the full detect condition in pm8921-charger-htc.c
					htc_batt_info.rep.pj_full: the full condition show to user space
					HW/Kenny feature implement:
						When plug AC/USB in or unplug/plug in PJ,
						MBI of main battery =100%,  power jacket > 95%, set PJ full*/
	...
	/* Open PJ_CHG for 0.1s to prevent 2.4V PJ can't be detected */
...
static void batt_worker(struct work_struct *work)
{
	...
	if (htc_batt_info.rep.charging_source > 0) {
		...
		/* check whether AICL is support by QCT PMIC or not */
		if(htc_batt_info.icharger &&
...
#endif
	/* STEP: fresh power_jacket information if power jacket exist (voltage) */
	if(htc_batt_info.icharger
